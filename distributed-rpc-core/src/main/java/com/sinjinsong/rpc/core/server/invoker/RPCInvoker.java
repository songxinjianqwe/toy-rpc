package com.sinjinsong.rpc.core.server.invoker;
...
public class RPCInvoker implements Runnable {
    ...
    public RPCInvoker(ChannelHandlerContext ctx, RPCRequest request, HandlerWrapper handlerWrapper) {
        this.ctx = ctx;
    ...
    public void run() {
        // callback的无需响应
        if(handlerWrapper.getRpcService() != null && handlerWrapper.getRpcService().callback()) {
            try {
                handle(request);
            } catch (Throwable t) {
                t.printStackTrace();
    ...
    private Object handle(RPCRequest request) throws Throwable {
        ...
        RPCService rpcService = handlerWrapper.getRpcService();
        if (rpcService != null && rpcService.callback()) {
            Class<?> interfaceClass = parameterTypes[rpcService.callbackParamIndex()];
            parameters[rpcService.callbackParamIndex()] = Proxy.newProxyInstance(
                    interfaceClass.getClassLoader(),
                    new Class<?>[]{interfaceClass},
                    new InvocationHandler() {
                        @Override
                        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
                            if (method.getName().equals(rpcService.callbackMethod())) {
                                // 创建并初始化 RPC 请求
